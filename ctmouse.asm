...
; DirtyRat specific changes by Scrap Computing 2023
;
...
DRTRATVER	equ <"0.1">		; dirty rat version
...
IO_address	dw ?
		...
		call_	mouseproc,MSLTproc	; never PS/2
		jmp	rethandler
IRQhandler	endp
		assume	ds:@TSRdata

UserIRQ		db 	3	; The user-defined IRQ number
UserIOAddr	dw 	2e8h	; The user-defined I/O address
...
disabledrv_1F	proc
		...
		call_	disableproc,disableUART	; can change ES!
...
;-----
		...
		jmp_	enableproc,enableUART	; can change ES!
...
;----- parse command line and find mouse
		...
		push	ax
		push 	cx
		mov	ax,[UserIOAddr] 	; Use the user-specified I/O Address
		mov	[IO_address],ax
	...
	;; I am not sure what this does:
		...
		mov	al,[UserIRQ] 		; IRQ set by the user
		mov	cl,al			; Save the IRQ number in cl
		mov	mouseinfo[0],al		; IRQ number
		add	al,'0'
		mov	[IRQno],al		; IRQNo='0'+IRQ
		sub	al,'0'
		add	al,8			; INT=IRQ+8
		mov	[IRQintnum],al
		mov	al,1
		...
		mov	al,2
		mov	mouseinfo[1],al 	; 2 = Microsoft
		mov	[mouseinfo1],al 	; 2 = serial

		mov	al,3			; Microsoft mouse = 3
		mov	[mousetype],al
...
;========================================================================
; Parse a number with up to 4 digits.
; Each digit is restricted to BL-BH values
; In:   DS:SI (null terminated string)
;       BL  - lower bound for digit, usually 0
;       BH  - upper bound for digit: 9 for decimal, 0xf for hex
; Out:  AX  - the number parsed
; Modf: none
; Call: parsedigit
parseNum	proc
		push	dx
		push	cx
		xor	dx,dx
@@nextChar:	call	parsedigit 	; Parsed digit is in AL
		jc	@@endOfString	; Break if end of string

		shl	dx,1  		; Shift left by 4 bits to make room for digit
		shl	dx,1
		shl	dx,1
		shl	dx,1
		jc	@@errTooLong	; If overflow, break on error
	
		add	dx,ax		; Accumulate the result into [UserIOAddr]
		j	@@nextChar	; Parse the next character

@@endOfString:	mov 	ax,dx
		pop	cx
		pop	dx
		ret

@@errTooLong:	mov	cx,dataref:E_argument ; 'Error: Invalid argument'
		j	BADOPTION
...
OPTABLE		cmOPTION <'O',OPT_Wheel,			@ret>
...
;========================================================================
; Parse the IO Address command line option
; In:   DS:SI (null terminated string)
; Out:  [UserIOAddr]
; Modf: none
; Call: parseNum
setIOAddress 	proc
		push 	ax
		push 	bx
		mov	bl,00h	; Allow hex 0-f, so lower bound is 0x0
		mov	bh,0fh	; Allow hex 0-f, so upper bound is 0xf
		call	parseNum
		mov	[UserIOAddr],ax
		pop 	bx
		pop	ax
...
setIRQ	 	proc
		push 	ax
		push 	bx
		mov	bl,0	; Allow decimal 0-9, so lower bound is 0
		mov	bh,9	; Allow decimal 0-9, so upper bound is 9
		call	parseNum
		mov	[UserIRQ],al
		pop 	bx
		pop	ax
